...
public class Controller {
	...
	public static void main(String [ ] args)
	{
		...
		if (args[0].equals("SHOT"))
		{
			...
			//return.. which one???//Game.getInstance().addShot(boolean aGoal, int aTime, Player aPlayer, Goalie aGoalie);
		}
		else if(args[0].equals("GOAL"))
		{
			//Player.getInstance().addShot( true, int aTime, Goalie aGoalie, Game aGame);
			//Game.getInstance().addShot(true, int aTime, Player aPlayer, Goalie aGoalie);
		}
		else if(args[0].equals("YELLOWCARD"))
		{
			//Game,getInstance()addInfraction(Infraction aInfraction, int index);
		}
